    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            ...
            <c:forEach items="${typeList}" var="pt">
                <option value="${pt.typeId}" <c:if test="${pt.typeId eq vo.typeid}">selected</c:if>>${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
                        ...
                        <div>
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    ...
                                    <c:forEach begin="${info.navigateFirstPage}" end="${info.navigateLastPage}" var="i">
...
<script type="text/javascript">
    ...
    function deleteBatch(page) {
            ...
            if(zhi.length==0){
                ...
                //拼接ID
                    ...
                    alert(str);
                    //发送ajax请求到服务器端
                    //window.location="${pageContext.request.contextPath}/prod/deleteBatch.action?str="+str+"&page="+page;
                    $.ajax({
    ...
    function del(pid,page) {
        ...
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            //window.location="${pageContext.request.contextPath}/prod/delete.action?pid="+pid+"&page="+page;
            //使用ajax删除数据
            $.ajax({
    ...
    function one(pid, ispage) {
        ...
        var lprice = $("#lprice").val();
        var hprice = $("#hprice").val();
        var str = "?pname="+pname+"&typeid="+typeid+"&lprice="+lprice+"&hprice="+hprice+"&page="+ispage+"&pid="+pid;
        location.href = "${pageContext.request.contextPath}/prod/getProd.action"+str;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
            ...
            success:function () {
                ...
                //load重新加载的方法 在重新加载product.jsp容器上的table
                $("#table").load("http://localhost:8080/admin/product.jsp #table");
            }
        })
    };
